## DigitalOcean’da Kubernetes Kluster Yaratish
...
Birinchi bo'lib, biz o'zimizning kompyuterimizda kubectl o'rnatishimiz kerak. Kubectl - bu Kubernetes klusterlarini boshqarish uchun ishlatiladigan komanda qatori vositasi. Kubectl'ni o'rnatish uchun quyidagi buyruqni terminalga kiriting:
...
## biz DigitalOceandagi kubernetes klusteriga ulanishimiz uchun quidagilarni bajarishimiz kerak:
1. DigitalOcean hisobingizga kiring.
2. "Kubernetes" bo'limiga o'ting.   
3. "Create Kubernetes Cluster" tugmasini bosing.
4. Kluster sozlamalarini tanlang
5. Kluster nomini kiriting va kerakli konfiguratsiyani tanlang (masalan, kluster o'lchami, mintaqa, va boshqalar).
6. "Create Cluster" tugmasini bosing va kluster yaratilishini kuting. Bu bir necha daqiqa davom etishi mumkin.
7. Kluster yaratilgandan so'ng, "Kubeconfig" faylini yuklab oling. Bu fayl sizning klusterga ulanish uchun kerak bo'ladi.
8. Terminalda quyidagi buyruqni kiriting va yuklab olingan kubeconfig faylini ko'rsating:

```bash
export KUBECONFIG=~/.kube/config
```
9. Endi siz kubectl yordamida klusterga ulanish va boshqarish imkoniyatiga egasiz. Masalan, klusterdagi tugunlarni ko'rish uchun quyidagi buyruqni kiriting:
...
```
Shunday qilibz biz o'zimiznug windows terminalimizda kubectl yordamida klusterimizga ulanish va boshqarish imkoniyatiga egamiz. Endi siz klusteringizda podlarni yaratish, xizmatlarni boshqarish va boshqa Kubernetes resurslarini boshqarish uchun kubectl'ni ishlatishingiz mumkin.
## DigitalOcean’da Kubernetes Kluster Yaratish va Public IP Qo'shish
### 1-bosqich: Kluster yaratish
DigitalOcean’da Kubernetes kluster yaratish uchun quyidagi amallarni bajaring:
1. DigitalOcean hisobingizga kiring.
2. "Kubernetes" bo'limiga o'ting.
3. "Create Kubernetes Cluster" tugmasini bosing.
4. Kluster sozlamalarini tanlang (masalan, kluster o'lchami, mintaqa, va boshqalar).
5. Kluster nomini kiriting.
6. "Create Cluster" tugmasini bosing va kluster yaratilishini kuting (bu bir necha daqiqa davom etishi mumkin).
### 2-bosqich: Kubeconfig faylini yuklab olish
Kluster yaratilgandan so'ng, "Kubeconfig" faylini yuklab oling. Bu fayl sizning klusterga ulanish uchun kerak bo'ladi.
### 3-bosqich: Kluster holatini tekshirish
Terminalda quyidagi buyruqni kiriting va yuklab olingan kubeconfig faylini ko'rsating:
```bash
export KUBECONFIG=~/.kube/config
```
Endi siz kubectl yordamida klusterga ulanish va boshqarish imkoniyatiga egasiz. Kluster holatini tekshirish uchun quyidagi buyruqni kiriting:
```bash
kubectl get nodes
kubectl get pods -A
``` 
...
Agar siz klusteringizga Public IP qo'shmoqchi bo'lsangiz, quyidagi amallarni bajaring:
1. DigitalOcean’da klusteringizni tanlang.
...
```
Yuqoridagi buyruqlardan birini bajarish orqali API Server manzilining Public IP bo'lishini tekshirishingiz mumkin. Agar manzil Public IP bo'lsa, demak siz muvaffaqiyatli klusteringizga Public IP qo'shganingizni anglatadi.
